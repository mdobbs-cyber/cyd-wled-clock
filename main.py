...
def main():
    ...
    # 3. Backlight PWM — allows brightness control from Settings screen
    # Frequency 1000 Hz, duty 0-1023 (10-bit on ESP32)
    bl_pwm = PWM(Pin(TFT_BL), freq=1000)
    # Set initial brightness from config (0-255 → 0-1023)
    init_bri = config.get('display_brightness', 200)
    bl_pwm.duty(int(init_bri / 255 * 1023))
    ...
    display = ili9341.ILI9341(
        ...
        dc=Pin(TFT_DC),
        rst=None,
        bl=None,          # backlight managed by PWM above, not the driver
    ...
    dashboard = app_dashboard.DashboardApp(display, config, bl_pwm)
